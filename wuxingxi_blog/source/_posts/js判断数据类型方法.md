...
alert(typeof NaN)   ------------> number //NaN是一种特殊的number,NaN与任何值都不相等,与自己也不相等
...
但是在判断引用类型的值时，常常会遇到一个问题：无论引用的是什么类型的对象，都会返回 "object"(当然还有 "function") 。有时候我们需要知道这个引用对象的类型是数组还是一个包装对象，这个时候 instanceof 就可以派上用场了。
```

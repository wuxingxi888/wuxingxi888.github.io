...
date: 2026-02-06 17:26:13
tags: 前端 AI编程
categories: 其他
top_img: https://cdn.jsdelivr.net/gh/wuxingxi888/CDN_IMG_BED/AI_coding_1.webp
cover: https://cdn.jsdelivr.net/gh/wuxingxi888/CDN_IMG_BED/AI_coding_1.webp
---

> 本架构示例项目：[AI Coding Guidelines](https://github.com/wuxingxi888/ai-coding-guidelines)

> 原文链接（掘金）：[前端 AI Coding 落地指南（一）架构篇](https://juejin.cn/post/7603347438013480995?utm_source=gold_browser_extension#heading-0)
>
> 转载声明：本文为转载整理内容，仅用于技术学习与交流；相关版权归原作者所有。如有侵权，请联系删除。
>
> 方案更新说明：文中多 Agent 配置方案已从“软链接共享”升级为“以 `.agents` 为单一事实源，通过 `scripts/sync-rules.js` 进行脚本物理分发同步”。
...
首先在项目根目录创建 **`.agents`**，仅在这里维护 `rules` 和 `skills`。然后通过 `scripts/sync-rules.js` 做**脚本分发同步**，而不是软链接：

- **唯一事实来源（Single Source of Truth）**：所有规范只在 `.agents` 修改。
- **物理同步分发**：脚本将 `.agents` 内容同步到各 IDE 目录（如 `.cursor`、`.claude`、`.trae`、`.windsurf`），避免软链接在 Git/Windows 场景下不稳定的问题。
- **自动触发 + 手动触发**：`npm install` 可通过 `postinstall` 自动执行同步；规则更新后也可手动执行 `node scripts/sync-rules.js`。
- **按 IDE 差异化适配**：例如 Cursor 同步后规则后缀转换为 `.mdc`，Windsurf/Trae 保持 `.md`，确保各工具“同一套规范、不同格式适配”。

这样你只维护 `.agents` 一份，就能稳定支持多种 AI Coding IDE 的一致协作。
